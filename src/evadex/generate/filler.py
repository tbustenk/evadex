...
_TEMPLATES: dict[PayloadCategory, list[str]] = {
    ...
    PayloadCategory.CHIPS_UID: [
        ...
        "CHIPS participant payment — UID {v} confirmed.",
    ],
    PayloadCategory.ISIN: [
        "Security ISIN: {v}.",
        "Trade executed for ISIN {v}, qty 5,000 shares.",
        "Settlement instruction — ISIN {v}, value date T+2.",
        "Position report: long 10,000 units ISIN {v}.",
        "Custody transfer: ISIN {v} moved to client account.",
        "ISIN {v} added to restricted securities list.",
        "Corporate action notification for ISIN {v}: 2-for-1 split.",
        "Collateral: ISIN {v} pledged, haircut 5%.",
    ],
    PayloadCategory.CUSIP_NUM: [
        "CUSIP {v} — equity security on file.",
        "Trade confirmation: CUSIP {v}, 1,000 shares at $148.50.",
        "DTC settlement for CUSIP {v}, contra-party BofA.",
        "Risk exposure report: CUSIP {v} position $2.4M.",
        "Restricted list — CUSIP {v} blackout period through 2024-06-30.",
        "Prospectus supplement filed for CUSIP {v}.",
    ],
    PayloadCategory.CINS_NUM: [
        "CINS {v} — international security registered in custody.",
        "Trade: CINS {v} purchased, 500 shares.",
        "Cross-border settlement — CINS {v} via Euroclear.",
        "Security identifier CINS {v} on restricted list.",
    ],
    PayloadCategory.SEDOL_NUM: [
        "SEDOL {v} — LSE-listed security.",
        "Trade: SEDOL {v}, 2,000 shares at GBP 12.40.",
        "Settlement via CREST for SEDOL {v}, counterparty Barclays.",
        "Corporate action: SEDOL {v} ex-dividend date 2024-05-09.",
        "Custody record — SEDOL {v}, 50,000 units held.",
    ],
    PayloadCategory.FIGI_NUM: [
        "Bloomberg FIGI: {v}.",
        "Global instrument identifier FIGI {v} on Bloomberg Terminal.",
        "OMS order tagged with FIGI {v} for regulatory reporting.",
        "TCA report: FIGI {v} — implementation shortfall 12 bps.",
        "Bloomberg data pull for FIGI {v}: last price $182.63.",
    ],
    PayloadCategory.LEI_NUM: [
        "Legal Entity Identifier: {v}.",
        "Counterparty LEI {v} verified against GLEIF registry.",
        "EMIR trade report: reporting entity LEI {v}.",
        "Swap confirmation — LEI {v} (buyer), LEI {v} (seller).",
        "KYC onboarding: entity LEI {v} confirmed active.",
        "MiFID II counterparty LEI: {v}.",
    ],
    PayloadCategory.TICKER_SYMBOL: [
        "Stock ticker {v} added to watchlist.",
        "Position: long {v}, 5,000 shares, avg cost $142.10.",
        "Alert: {v} crossed 52-week high of $189.30.",
        "Options activity: {v} Jan 2025 $200 calls, volume 12,400.",
        "Portfolio weight: {v} 8.2% as of end of month.",
        "Earnings release for {v} beats consensus by $0.18 EPS.",
    ],
    PayloadCategory.REUTERS_RIC: [
        "Reuters RIC {v} — real-time quote on Refinitiv.",
        "Market data subscription: RIC {v}, tick data from open.",
        "VWAP analysis for RIC {v}: 10:00–14:30 EST.",
        "Quote: {v} last 148.50, bid 148.48, ask 148.51.",
        "News headline tagged with RIC {v}.",
    ],
    PayloadCategory.VALOR_NUM: [
        "Swiss Valor {v} — SIX Swiss Exchange listing.",
        "Valor-Nummer {v} for Euroclear settlement.",
        "Trade confirmation: Valor {v}, 1,000 CHF 98.20.",
        "Swiss custodian reference: Valor {v} in depot.",
    ],
    PayloadCategory.WKN_NUM: [
        "German WKN {v} — Xetra order entry.",
        "Wertpapierkennnummer {v}: trade at EUR 82.40.",
        "CBF settlement for WKN {v}, value date T+2.",
        "Frankfurt exchange trade: WKN {v}, 2,500 shares.",
    ],
    PayloadCategory.MT103_REF: [
        "SWIFT MT103 payment reference: {v}.",
        "Incoming wire — MT103 sender reference {v}, USD 1,250,000.",
        "Cross-border payment instruction ref: {v}.",
        "SWIFT message field 20: {v} — beneficiary bank confirmed.",
        "Correspondent bank reconciliation: MT103 ref {v}.",
    ],
    PayloadCategory.MIFID_TX_ID: [
        "MiFID II transaction reference: {v}.",
        "Regulatory trade report — transaction ID {v} filed with ESMA.",
        "ARM submission: transaction reference number {v}.",
        "MiFID trade report ref {v} — execution venue: XLON.",
    ],
    PayloadCategory.SEPA_REF: [
        "SEPA creditor reference: {v}.",
        "SEPA direct debit — mandate reference {v}, EUR 450.00.",
        "Remittance information: creditor ref {v}.",
        "SEPA payment: structured creditor reference {v} per ISO 11649.",
    ],
    PayloadCategory.FEDWIRE_IMAD: [
        "Fedwire IMAD: {v}.",
        "Inbound Fedwire — IMAD {v}, USD 5,000,000 received.",
        "Fedwire payment trace — IMAD {v}, originator JPMorgan.",
        "Wire reconciliation: Fedwire IMAD {v} matched to book entry.",
    ],
