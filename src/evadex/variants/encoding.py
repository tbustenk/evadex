...
class EncodingGenerator(BaseVariantGenerator):
    ...
    def _base64_variants(self, value: str) -> Iterator[Variant]:
        ...
        yield self._make_variant(
            b64, "base64_standard", "Value base64-encoded (standard alphabet)"
        )
        ...
        if b64url != b64:
            yield self._make_variant(
                b64url, "base64_urlsafe", "Value base64-encoded (URL-safe alphabet)"
            )
        ...
        if b64_nopad != b64:
            yield self._make_variant(
                b64_nopad, "base64_no_padding", "Value base64-encoded, padding stripped"
            )
        ...
        if len(b64) > 76:
            ...
            yield self._make_variant(
                ...
                "Base64 with MIME-style line breaks every 76 characters",
            )
        ...
        yield self._make_variant(
            partial, "base64_partial", "First half literal, second half base64-encoded"
        )
    ...
    def _base32_variants(self, value: str) -> Iterator[Variant]:
        ...
        yield self._make_variant(
            b32, "base32_standard", "Value base32-encoded (standard alphabet A–Z, 2–7)"
        )
        ...
        if b32_nopad != b32:
            yield self._make_variant(
                b32_nopad, "base32_no_padding", "Value base32-encoded, padding stripped"
            )

        # Lowercase
        b32_lower = b32.lower()
        yield self._make_variant(
            b32_lower, "base32_lowercase", "Value base32-encoded, lowercase alphabet"
        )
        ...
        yield self._make_variant(
            b32hex,
            "base32_hex_alphabet",
            "Value base32-encoded with extended hex alphabet (0–9, A–V)",
        )
    ...
    def _hex_variants(self, value: str) -> Iterator[Variant]:
        ...
        yield self._make_variant(
            hex_lower, "hex_lowercase", "Value hex-encoded, lowercase (e.g. 34313131…)"
        )

        # Uppercase hex
        hex_upper = hex_lower.upper()
        yield self._make_variant(
            hex_upper, "hex_uppercase", "Value hex-encoded, uppercase"
        )

        # \\xNN escape sequence per byte
        hex_escaped = "".join(f"\\x{b:02x}" for b in raw)
        yield self._make_variant(
            hex_escaped, "hex_escaped_bytes", r"Value as \xNN escape sequences per byte"
        )
        ...
        yield self._make_variant(
            hex_0x,
            "hex_0x_integer",
            "Value encoded as a single 0x-prefixed hex integer",
        )
        ...
        yield self._make_variant(
            hex_spaced,
            "hex_spaced_bytes",
            "Value as space-separated hex bytes (hex dump style)",
        )
    ...
    def _rot13_variants(self, value: str) -> Iterator[Variant]:
        ...
        yield self._make_variant(
            rot, "rot13", "ROT13 applied to all alphabetic characters"
        )
        ...
        if rot_letters_only != rot:
            yield self._make_variant(
                rot_letters_only,
                "rot13_letters_only",
                "ROT13 applied to letters only, digits and symbols unchanged",
            )
    ...
    def _reversed_variants(self, value: str) -> Iterator[Variant]:
        ...
        if within != raw and within != rev:
            yield self._make_variant(
                ...
                "Digits reversed within each 4-character group",
            )
        ...
        if group_order != raw and group_order != within:
            yield self._make_variant(
                ...
                "4-character groups reversed in order",
            )
    ...
    def _double_url_encoding(self, value: str) -> Iterator[Variant]:
        ...
        yield self._make_variant(
            double, "double_url_encoding", "Double percent-encoding (%XX → %25XX)"
        )
        ...
        if selective != double:
            yield self._make_variant(
                selective,
                "double_url_encoding_digits",
                "Double percent-encoding on digits only",
            )
    ...
    def _mixed_normalization(self, value: str) -> Iterator[Variant]:
        ...
        if nfkd != value:
            yield self._make_variant(
                nfkd,
                "nfkd_normalization",
                "Unicode NFKD normalization (aggressive decomposition)",
            )
