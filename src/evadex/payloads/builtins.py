...
BUILTIN_PAYLOADS = [
    ...
    Payload(
        "340000136", PayloadCategory.US_PASSPORT, "US Passport number"
    ),  # fires as CUSIP (9-digit collision); context_required for USA Passport pattern
    Payload("123 456 78", PayloadCategory.AU_TFN, "Australia TFN"),
    Payload(
        "86095742719", PayloadCategory.DE_TAX_ID, "Germany Steuer-IdNr"
    ),  # fires as Geohash (11-digit/charset collision); context_required for Germany Tax ID
    Payload(
        "282097505604213", PayloadCategory.FR_INSEE, "France INSEE (NIR)"
    ),  # fires as PAN (15-digit collision); context_required for France NIR
    # --- Banking ---
    ...
    Payload(
        "021000021", PayloadCategory.ABA_ROUTING, "ABA routing number"
    ),  # fires as CUSIP (9-digit collision); context_required for ABA Routing Number
    # --- Cryptocurrency ---
    Payload(
    ...
    Payload(
        "sk_test_4eC39HqLyjWDarjtT7en6bh8Xy9mPqZ",
        PayloadCategory.STRIPE_KEY,
        "Stripe test secret key",
    ),
    Payload(
        "xoxb-EXAMPLE-BOTTOKEN-abc123def",
    ...
    Payload(
        "TOP SECRET", PayloadCategory.CLASSIFICATION, "Top Secret classification label"
    ),
    ...
    Payload(
        "eyJhbGciOiJIUzI1NiJ9.dGVzdHBheWxvYWRyYW5kb20xMjM0NTY3ODkw.abc123XYZdef456UVWghi789RSTjkl",
    ...
    Payload(
        "dXNlcm5hbWU6c3VwZXJfc2VjcmV0X3Bhc3N3b3JkXzEyMzQ1Ng==",
    ...
    Payload(
        "DATABASE_PASSWORD=xK9mP2nL4qR7vT1wY6uI0oE3sA5dF8hJ",
    ...
    Payload(
        "api_key: xK9mP2nL4qR7vT1wY6uI0oE3sA5dF8hJ",
    ...
    Payload(
        "1234-567-890-AB", PayloadCategory.CA_ONTARIO_HEALTH, "Ontario health card"
    ),
    ...
    Payload(
        ...
        "Quebec driver's licence (letter + 12 digits)",
    ),
    Payload(
        "A1234-56789-01234", PayloadCategory.CA_ON_DRIVERS, "Ontario driver's licence"
    ),
    Payload(
        "1234567", PayloadCategory.CA_BC_DRIVERS, "British Columbia driver's licence"
    ),
    ...
    Payload(
        "A123456789", PayloadCategory.CA_NL_DRIVERS, "Newfoundland driver's licence"
    ),
    # --- Canadian corporate identifiers ---
    Payload(
        "111222333", PayloadCategory.CA_BUSINESS_NUMBER, "Canadian Business Number (BN)"
    ),
    Payload(
        "111222333RT0001", PayloadCategory.CA_GST_HST, "Canadian GST/HST registration"
    ),
    Payload(
        "12345-678",
        PayloadCategory.CA_TRANSIT_NUMBER,
        "Canadian transit/routing number",
    ),
    ...
    Payload(
        "12-3456789", PayloadCategory.US_EIN, "US EIN (Employer Identification Number)"
    ),
    Payload(
        "1EG4-TE5-MK72", PayloadCategory.US_MBI, "US Medicare Beneficiary Identifier"
    ),
    ...
    Payload(
        "AB123456C", PayloadCategory.US_DL, "Idaho DL (2 letters + 6 digits + letter)"
    ),
    ...
    Payload(
        "123AB1234", PayloadCategory.US_DL, "Iowa DL (3 digits + 2 letters + 4 digits)"
    ),
    ...
    Payload(
        "M123456789012", PayloadCategory.US_DL, "Minnesota DL (letter + 12 digits)"
    ),
    ...
    Payload(
        "A12345678901234", PayloadCategory.US_DL, "New Jersey DL (letter + 14 digits)"
    ),
    ...
    Payload(
        "AAA123456", PayloadCategory.US_DL, "North Dakota DL (3 letters + 6 digits)"
    ),
    ...
    Payload(
        "ABCDE12345",
        PayloadCategory.US_DL,
        "Washington DL (5 letters + 5 alphanumeric)",
    ),
    Payload("W123456", PayloadCategory.US_DL, "West Virginia DL (letter + 6 digits)"),
    Payload(
        "A1234567890123", PayloadCategory.US_DL, "Wisconsin DL (letter + 13 digits)"
    ),
    ...
    Payload(
        ...
        "German Personalausweis (9-char alphanumeric)",
    ),
    Payload(
        "880692310285",
        PayloadCategory.FR_CNI,
        "French Carte Nationale d'Identité (CNI)",
    ),
    ...
    Payload(
        ...
        "Session token (32-char hex)",
    ),
    Payload(
        "0123456789ABCDEF",
        PayloadCategory.PIN_BLOCK,
        "PIN block (ISO format 0, 16 hex chars)",
    ),
    Payload(
        "12345678-ABCD-1234-EFGH-123456789ABC",
        PayloadCategory.BIOMETRIC_ID,
        "Biometric identifier (UUID-style)",
    ),
    Payload("12/26", PayloadCategory.CARD_EXPIRY, "Card expiration date MM/YY"),
    Payload(
        "%B4532015112830366^SMITH/JOHN^2512101000000000?",
        PayloadCategory.CARD_TRACK,
        "Card track 1 data",
    ),
    Payload(
        "⑈021000021⑈ 123456789012 1234", PayloadCategory.MICR, "MICR check line data"
    ),
    Payload(
        "Company Confidential",
        PayloadCategory.CORP_CLASSIFICATION,
        "Corporate confidential classification label",
    ),
    Payload(
        "USD 12,345.67",
    ...
    Payload(
        "89014103211118510720", PayloadCategory.ICCID, "SIM card ICCID (20 digits)"
    ),
    Payload(
        "john.smith@mit.edu",
    ...
    Payload(
        "40.7128,-74.0060", PayloadCategory.GPS_COORDS, "GPS coordinates (lat/lon)"
    ),
    Payload(
        "POL123456789", PayloadCategory.INSURANCE_POLICY, "Insurance policy number"
    ),
    Payload(
        "ACCT12345678", PayloadCategory.BANK_REF, "Internal bank account reference"
    ),
    Payload("1:24-cv-12345", PayloadCategory.LEGAL_CASE, "Federal civil case number"),
    Payload(
        "ABCD00123456789012345678",
        PayloadCategory.LOAN_NUMBER,
        "Loan/mortgage identifier",
    ),
    Payload("0069-3190-03", PayloadCategory.NDC_CODE, "National Drug Code (NDC)"),
    Payload(
        "John Smith", PayloadCategory.CARDHOLDER_NAME, "Cardholder name (PCI sensitive)"
    ),
    ...
    Payload(
        ...
        "Privacy/compliance classification label",
    ),
    Payload(
    ...
    Payload(
        ...
        "Anti-money laundering case identifier",
    ),
    Payload(
        "US0378331005",
    ...
    Payload(
        "@johnsmith", PayloadCategory.TWITTER_HANDLE, "Twitter/X social media handle"
    ),
    Payload(
        "Confidential Supervisory Information",
        PayloadCategory.SUPERVISORY_INFO,
        "Confidential supervisory information label",
    ),
    Payload(
        "https://admin:password123@example.com/api",
        PayloadCategory.URL_WITH_CREDS,
        "URL containing embedded credentials",
    ),
    Payload(
        "1HGBH41JXMN109186", PayloadCategory.VIN, "Vehicle Identification Number (VIN)"
    ),
    Payload(
        "20240101AAAA12345678001234",
        PayloadCategory.FEDWIRE_IMAD,
        "Fedwire IMAD (Input Message Accountability Data)",
    ),
    # ── Wire Transfer Data ────────────────────────────────────────────────────
    Payload(
        "123456ABCD", PayloadCategory.CHIPS_UID, "CHIPS UID (6 digits + 4 alphanumeric)"
    ),
    Payload(
        "0001JPMC", PayloadCategory.CHIPS_UID, "CHIPS UID — JPMorgan Chase participant"
    ),
    Payload(
        "WIREREF1234567890ABCD",
        PayloadCategory.WIRE_REF,
        "Wire Reference Number (16-35 uppercase alphanumeric)",
    ),
    Payload(
        "RF18539007547034",
        PayloadCategory.SEPA_REF,
        "SEPA creditor reference (RF-format, ISO 11649)",
    ),
    Payload(
        "SEPAREF123456",
        PayloadCategory.SEPA_REF,
        "SEPA reference (legacy alphanumeric format)",
    ),
    Payload(
        "011234567890123",
        PayloadCategory.ACH_TRACE,
        "ACH Trace Number (15 digits, valid routing prefix)",
    ),
    ...
    Payload(
        ...
        "USA Routing Number (9-digit, non-ABA prefix)",
    ),
    Payload(
        "(555) 867-5309",
        PayloadCategory.US_PHONE,
        "US Phone Number (local format without country code)",
    ),
    # --- Africa ---
    ...
    Payload(
        "12345678901", PayloadCategory.NG_BVN, "Nigeria Bank Verification Number (BVN)"
    ),
    ...
    Payload(
        "85.01.01-234.56", PayloadCategory.BE_NRN, "Belgium National Register Number"
    ),
    ...
    Payload(
        "38501010002", PayloadCategory.EE_IK, "Estonia personal identification code"
    ),
    Payload("DE123456789", PayloadCategory.EU_VAT, "EU VAT number (Germany example)"),
    Payload(
        "01018512345", PayloadCategory.GR_AMKA, "Greece AMKA social security number"
    ),
    Payload(
        "123 456 789", PayloadCategory.HU_TAJ, "Hungary TAJ social security number"
    ),
    ...
    Payload(
        "1850101123456", PayloadCategory.RO_CNP, "Romania CNP personal numeric code"
    ),
    ...
    Payload(
        "123.456.789-0", PayloadCategory.CO_CEDULA, "Colombia cedula de ciudadania"
    ),
    ...
    Payload(
        "MO1234567",
        PayloadCategory.MA_PASSPORT,
        "Morocco passport (2 letters + 7 digits)",
    ),
    Payload("12345679", PayloadCategory.MA_TAX_ID, "Morocco tax ID (8 digits)"),
    Payload(
        "AAA123456789",
        PayloadCategory.NG_DL,
        "Nigeria driver licence (3+ letters + 5-9 digits)",
    ),
    Payload("12345678902", PayloadCategory.NG_NIN, "Nigeria NIN (11 digits)"),
    Payload(
        "A12345679", PayloadCategory.NG_PASSPORT, "Nigeria passport (letter + 8 digits)"
    ),
    Payload("1234567890124", PayloadCategory.NG_TIN, "Nigeria TIN (13 digits)"),
    Payload(
        "12345678901234ABCDE",
        PayloadCategory.NG_VOTER,
        "Nigeria voter card (19 alphanumeric)",
    ),
    Payload(
        "1234567890AB", PayloadCategory.ZA_DL, "South Africa DL (10 digits + 2 letters)"
    ),
    Payload("A12345679", PayloadCategory.ZA_PASSPORT, "South Africa passport"),
    Payload(
        "TZ1234567",
        PayloadCategory.TZ_PASSPORT,
        "Tanzania passport (2 letters + 7 digits)",
    ),
    Payload("123456790", PayloadCategory.TZ_TIN, "Tanzania TIN (9 digits)"),
    Payload(
        "A123457", PayloadCategory.TN_PASSPORT, "Tunisia passport (letter + 6 digits)"
    ),
    Payload(
        "A1234568", PayloadCategory.UG_PASSPORT, "Uganda passport (letter + 7-8 digits)"
    ),
    ...
    Payload(
        "BD1234567",
        PayloadCategory.BD_PASSPORT,
        "Bangladesh passport (2 letters + 7 digits)",
    ),
    Payload("123456789013", PayloadCategory.BD_TIN, "Bangladesh TIN (12 digits)"),
    # ── Asia-Pacific — China ────────────────────────────────────────────────
    Payload(
        "E12345678",
        PayloadCategory.CN_PASSPORT,
        "China passport (E/G/D + optional letter + 7-8 digits)",
    ),
    Payload(
        "110105199001011234", PayloadCategory.CN_RID, "China resident ID (18-digit)"
    ),
    Payload(
        "12345674",
        PayloadCategory.MO_ID,
        "Macau ID (starts 1/5/7/8 + 6 digits + check)",
    ),
    Payload(
        "A123456789",
        PayloadCategory.TW_NID,
        "Taiwan national ID (letter + [12489] + 8 digits)",
    ),
    # ── Asia-Pacific — India ────────────────────────────────────────────────
    ...
    Payload(
        "1312345678", PayloadCategory.IN_RATION_CARD, "India ration card (2 + 8 digits)"
    ),
    Payload(
        "ABC1234568",
        PayloadCategory.IN_VOTER_ID,
        "India voter ID (3 letters + 7 digits)",
    ),
    # ── Asia-Pacific — Indonesia ────────────────────────────────────────────
    Payload(
        "12.345.678.9-012.345", PayloadCategory.ID_NPWP, "Indonesia NPWP tax number"
    ),
    ...
    Payload(
        "12345679", PayloadCategory.JP_HEALTH_INS, "Japan health insurance (8 digits)"
    ),
    Payload(
        "12345678902", PayloadCategory.JP_JUMINHYO, "Japan juminhyo code (11 digits)"
    ),
    Payload(
        "JP1234567",
        PayloadCategory.JP_PASSPORT,
        "Japan passport (2 letters + 7 digits)",
    ),
    Payload(
        "AB12345679CD",
        PayloadCategory.JP_RESIDENCE_CARD,
        "Japan residence card (2+8+2)",
    ),
    # ── Asia-Pacific — Malaysia ─────────────────────────────────────────────
    Payload(
        "A12345679",
        PayloadCategory.MY_PASSPORT,
        "Malaysia passport (letter + 8 digits)",
    ),
    # ── Asia-Pacific — New Zealand ──────────────────────────────────────────
    Payload("AB123457", PayloadCategory.NZ_DL, "New Zealand DL (2 letters + 6 digits)"),
    Payload(
        "ABC1235", PayloadCategory.NZ_NHI, "New Zealand NHI (3 letters + 4 digits)"
    ),
    Payload("NZ123457", PayloadCategory.NZ_PASSPORT, "New Zealand passport"),
    # ── Asia-Pacific — Pakistan ─────────────────────────────────────────────
    Payload(
        "12346-1234568-1",
        PayloadCategory.PK_NICOP,
        "Pakistan NICOP (same format as CNIC)",
    ),
    Payload(
        "PK1234568",
        PayloadCategory.PK_PASSPORT,
        "Pakistan passport (2 letters + 7 digits)",
    ),
    ...
    Payload(
        "S1234568A", PayloadCategory.SG_DL, "Singapore DL (NRIC-format starts with S/T)"
    ),
    Payload("F1234568A", PayloadCategory.SG_FIN, "Singapore FIN (F/G/M prefix)"),
    Payload(
        "E1234568A",
        PayloadCategory.SG_PASSPORT,
        "Singapore passport (letter + 7 + letter)",
    ),
    ...
    Payload(
        "TH1234568",
        PayloadCategory.TH_PASSPORT,
        "Thailand passport (2 letters + 7 digits)",
    ),
    Payload("1234567890125", PayloadCategory.TH_TAX_ID, "Thailand tax ID (13 digits)"),
    # ── Asia-Pacific — Vietnam ──────────────────────────────────────────────
    Payload(
        "A12345679", PayloadCategory.VN_PASSPORT, "Vietnam passport (letter + 8 digits)"
    ),
    ...
    Payload(
        ...
        "HSM key (56 hex chars)",
    ),
    # ── Card Track Data ──────────────────────────────────────────────────────
    Payload(
        ";4532015112830366=25121010000000000?",
    ...
    Payload(
        "123456789012345",
        PayloadCategory.CASHIER_CHECK,
        "Cashier check number (15 digits)",
    ),
    Payload("12345", PayloadCategory.CHECK_NUMBER, "Check number (5 digits)"),
    # ── Cloud Provider Secrets ──────────────────────────────────────────────
    Payload(
        "wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY",
        PayloadCategory.AWS_SECRET_KEY,
        "AWS secret access key (40-char base64)",
    ),
    Payload(
        "AIzaSyDt0Y3QL5dFX6iFwIm6JFxE12345678901",
        PayloadCategory.GOOGLE_API_KEY,
        "Google API key (AIza + 35 chars)",
    ),
    # ── Code Platform Secrets ───────────────────────────────────────────────
    Payload(
        "gho_ABCDEFGHIJKLMNOPQRSTUVWXYZabcdef01",
        PayloadCategory.GITHUB_OAUTH,
        "GitHub OAuth token (gho_ + 36 chars)",
    ),
    Payload(
        "github_pat_ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz01234567",
        PayloadCategory.GITHUB_PAT,
        "GitHub fine-grained PAT",
    ),
    Payload(
        "npm_ABCDEFGHIJKLMNOPQRSTUVWXYZabcdef01",
        PayloadCategory.NPM_TOKEN,
        "NPM access token (npm_ + 36 chars)",
    ),
    Payload(
        "pypi-AgEIcHlwaS5vcmcCJABCDEFGHIJKLMNOPQRSTUVWXYZabcde",
        PayloadCategory.PYPI_TOKEN,
        "PyPI API token",
    ),
    # ── Cryptocurrency ──────────────────────────────────────────────────────
    Payload(
        "bc1qar0srrr7xfkvy5l643lydnw9re59gtzzwf5mdq",
        PayloadCategory.BITCOIN_BECH32,
        "Bitcoin Bech32 (native SegWit) address",
    ),
    Payload(
        "qpm2qsznhks23z7629mms6s4cwef74vcwvy22gdx6a",
        PayloadCategory.BITCOIN_CASH,
        "Bitcoin Cash address (q-prefix + 41 chars)",
    ),
    Payload(
        "LdP8Qox1VAhCzLJNqrr74YovaWYyNBUWvL",
        PayloadCategory.LITECOIN,
        "Litecoin address (L/M prefix + 26-33 base58)",
    ),
    Payload(
        "44ABCDEFGHJKMNPQRSTUVWXYZabcdefghjkmnpqrstuvwxyz1234567890ABCDEFGHJKMNPQRSTUVWXYZabcdefghjkmnpqrs",
        PayloadCategory.MONERO,
        "Monero address (95 chars starting with 4[0-9AB])",
    ),
    Payload(
        "r9cZA1mLK5R5Am25ArfXFmqgNwjZgnfk59",
        PayloadCategory.RIPPLE,
        "Ripple/XRP address (r + 34 base58 chars)",
    ),
    # ── Payment Service Secrets ─────────────────────────────────────────────
    Payload(
        "pk_test_4eC39HqLyjWDarjtT7en6bh8Xy9mPqZaB1C2D3",
        PayloadCategory.STRIPE_PK,
        "Stripe publishable key",
    ),
    # ── Generic Secrets ─────────────────────────────────────────────────────
    Payload(
        "Bearer eyJhbGciOiJIUzI1NiJ9.dGVzdA.abc123",
        PayloadCategory.BEARER_TOKEN,
        "Bearer token in Authorization header",
    ),
    Payload(
        "postgresql://admin:s3cr3tP@ssw0rd@db.example.com:5432/prod",
        PayloadCategory.DB_CONNECTION_STRING,
        "Database connection string with credentials",
    ),
    Payload(
        "-----BEGIN RSA PRIVATE KEY-----",
        PayloadCategory.PRIVATE_KEY,
        "PEM private key header",
    ),
    # ── Messaging Service Secrets ───────────────────────────────────────────
    Payload(
        "key-3ax6xnjp29jd6fds4gc373sgvjxteol0",
        PayloadCategory.MAILGUN_KEY,
        "Mailgun API key (key- + 32 chars)",
    ),
    Payload(
        "SG.ABCDEFGHIJKLMNOPQRSTUVWX.ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqr",
        PayloadCategory.SENDGRID_KEY,
        "SendGrid API key",
    ),
    Payload(
        "xoxp-EXAMPLE-USERTOKEN-abc123def456",
        PayloadCategory.SLACK_USER_TOKEN,
        "Slack user token (xoxp- prefix)",
    ),
    Payload(
        "https://hooks.slack.com/services/TTEST/BTEST/TESTTOKEN",
        PayloadCategory.SLACK_WEBHOOK,
        "Slack webhook URL (test pattern only)",
    ),
    Payload(
        "SK" + "deadbeef" * 4,
        PayloadCategory.TWILIO_KEY,
        "Twilio API key (SK + 32 hex, test pattern)",
    ),
    # ── Contact Information ──────────────────────────────────────────────────
    Payload("192.168.100.200", PayloadCategory.IPV4_ADDRESS, "IPv4 address"),
    Payload(
        "2001:0db8:85a3:0000:0000:8a2e:0370:7334",
        PayloadCategory.IPV6_ADDRESS,
        "IPv6 address (full notation)",
    ),
    Payload(
        "00:1A:2B:3C:4D:5E", PayloadCategory.MAC_ADDRESS, "MAC address (colon notation)"
    ),
    # ── Device Identifiers ──────────────────────────────────────────────────
    Payload(
        "12345678-ABCD-EF01-2345-67890ABCDEF0",
        PayloadCategory.IDFA_IDFV,
        "Apple IDFA/IDFV (uppercase hex UUID)",
    ),
    Payload(
        "35-209900-176148-1", PayloadCategory.IMEI, "IMEI (15 digits with separators)"
    ),
    Payload(
        "35-209900-176148-23",
        PayloadCategory.IMEISV,
        "IMEISV (16 digits with separators)",
    ),
    ...
    Payload(
        "CLM123456789",
        PayloadCategory.INSURANCE_CLAIM,
        "Insurance claim number (3 letters + 9 digits)",
    ),
    # ── Internal Banking ────────────────────────────────────────────────────
    Payload("TLR12345", PayloadCategory.TELLER_ID, "Bank teller ID (context required)"),
    # ── Corporate Classification ─────────────────────────────────────────────
    Payload(
        "Not for Distribution", PayloadCategory.CORP_DND, "Do not distribute label"
    ),
    Payload(
        "Embargoed Information",
        PayloadCategory.CORP_EMBARGOED,
        "Embargoed material label",
    ),
    Payload("Eyes Only", PayloadCategory.CORP_EYES_ONLY, "Eyes Only label"),
    Payload(
        "Highly Confidential",
        PayloadCategory.CORP_HIGHLY_CONF,
        "Highly Confidential label",
    ),
    Payload(
        "Internal Use Only", PayloadCategory.CORP_INTERNAL_ONLY, "Internal Only label"
    ),
    Payload("Need to Know", PayloadCategory.CORP_NEED_TO_KNOW, "Need to Know label"),
    Payload(
        "Proprietary Information",
        PayloadCategory.CORP_PROPRIETARY,
        "Proprietary information label",
    ),
    ...
    Payload(
        ...
        PayloadCategory.DC_CLASSIFIED_CONF,
        "Classified Confidential label",
    ),
    ...
    Payload(
        "A1234568", PayloadCategory.AT_PASSPORT, "Austria passport (letter + 7 digits)"
    ),
    Payload(
        "12-345-6790", PayloadCategory.AT_TAX_NUM, "Austria tax number (2-3-4 digits)"
    ),
    # ── Europe — Belgium ─────────────────────────────────────────────────────
    Payload(
        "1234567891", PayloadCategory.BE_DL, "Belgium driver's licence (10 digits)"
    ),
    Payload(
        "AB123457",
        PayloadCategory.BE_PASSPORT,
        "Belgium passport (2 letters + 6 digits)",
    ),
    ...
    Payload(
        "A1234568", PayloadCategory.CY_PASSPORT, "Cyprus passport (letter + 7-8 digits)"
    ),
    ...
    Payload(
        "AB1234568",
        PayloadCategory.EE_PASSPORT,
        "Estonia passport (2 letters + 7 digits)",
    ),
    # ── Europe — Finland ────────────────────────────────────────────────────
    Payload("12345679", PayloadCategory.FI_DL, "Finland DL (8 digits)"),
    Payload(
        "AB1234568",
        PayloadCategory.FI_PASSPORT,
        "Finland passport (2 letters + 7 digits)",
    ),
    # ── Europe — France ──────────────────────────────────────────────────────
    Payload(
        "12AB34568",
        PayloadCategory.FR_DL,
        "France DL (2 digits + 2 letters + 5 digits)",
    ),
    Payload(
        "12AB34569", PayloadCategory.FR_PASSPORT, "France passport (same pattern as DL)"
    ),
    Payload("FR7630006000011234567890189", PayloadCategory.FR_IBAN, "France IBAN"),
    # ── Europe — Germany ─────────────────────────────────────────────────────
    Payload("B073RD3J1", PayloadCategory.DE_DL, "Germany DL (11 alphanumeric)"),
    Payload(
        "C1234567R",
        PayloadCategory.DE_PASSPORT,
        "Germany passport (C + 8 alphanumeric)",
    ),
    Payload(
        "12010185A001", PayloadCategory.DE_SOCIAL_INS, "Germany social insurance number"
    ),
    Payload("DE89370400440532013001", PayloadCategory.DE_IBAN, "Germany IBAN"),
    # ── Europe — Greece ──────────────────────────────────────────────────────
    ...
    Payload(
        "AC123457", PayloadCategory.GR_ID_CARD, "Greece ID card (2 letters + 6 digits)"
    ),
    Payload(
        "AB1234568",
        PayloadCategory.GR_PASSPORT,
        "Greece passport (2 letters + 7 digits)",
    ),
    # ── Europe — Hungary ─────────────────────────────────────────────────────
    Payload("AB123457", PayloadCategory.HU_DL, "Hungary DL (2 letters + 6 digits)"),
    Payload(
        "AB1234568",
        PayloadCategory.HU_PASSPORT,
        "Hungary passport (2 letters + 6-7 digits)",
    ),
    ...
    # ── Europe — Iceland ─────────────────────────────────────────────────────
    Payload(
        "A1234568", PayloadCategory.IS_PASSPORT, "Iceland passport (letter + 7 digits)"
    ),
    # ── Europe — Ireland ─────────────────────────────────────────────────────
    ...
    Payload(
        "AB1234568",
        PayloadCategory.IE_PASSPORT,
        "Ireland passport (2 letters + 7 digits)",
    ),
    # ── Europe — Italy ──────────────────────────────────────────────────────
    ...
    Payload(
        "AB1234568",
        PayloadCategory.IT_PASSPORT,
        "Italy passport (2 letters + 7 digits)",
    ),
    ...
    Payload(
        "AB1234568",
        PayloadCategory.LV_PASSPORT,
        "Latvia passport (2 letters + 7 digits)",
    ),
    ...
    Payload(
        "AB123457",
        PayloadCategory.LU_PASSPORT,
        "Luxembourg passport (2 letters + 6 digits)",
    ),
    # ── Europe — Malta ───────────────────────────────────────────────────────
    ...
    Payload(
        "41018512346",
        PayloadCategory.NO_D_NUMBER,
        "Norway D-number (4-7 in first position)",
    ),
    ...
    Payload(
        "ABC123457", PayloadCategory.PL_ID_CARD, "Poland ID card (3 letters + 6 digits)"
    ),
    Payload("123-456-13-12", PayloadCategory.PL_NIP, "Poland NIP tax number"),
    Payload(
        "AB1234568",
        PayloadCategory.PL_PASSPORT,
        "Poland passport (2 letters + 7 digits)",
    ),
    ...
    Payload(
        "AB123457",
        PayloadCategory.PT_PASSPORT,
        "Portugal passport (1-2 letters + 6 digits)",
    ),
    # ── Europe — Romania ─────────────────────────────────────────────────────
    ...
    Payload(
        "AB123458",
        PayloadCategory.SK_PASSPORT,
        "Slovakia passport (2 letters + 6 digits)",
    ),
    # ── Europe — Slovenia ────────────────────────────────────────────────────
    Payload("12345679", PayloadCategory.SI_DL, "Slovenia DL (8 digits)"),
    Payload(
        "AB1234568",
        PayloadCategory.SI_PASSPORT,
        "Slovenia passport (2 letters + 7 digits)",
    ),
    ...
    Payload(
        "X1234568L", PayloadCategory.ES_NIE, "Spain NIE (X/Y/Z + 7 digits + letter)"
    ),
    ...
    Payload("ES9121000418450200051332", PayloadCategory.ES_IBAN, "Spain IBAN"),
    # ── Europe — Sweden ──────────────────────────────────────────────────────
    ...
    Payload(
        "A1234568",
        PayloadCategory.CH_PASSPORT,
        "Switzerland passport (letter + 7 digits)",
    ),
    Payload(
        "CHE-123.456.789", PayloadCategory.CH_UID, "Switzerland UID (CHE-NNN.NNN.NNN)"
    ),
    # ── Europe — Turkey ──────────────────────────────────────────────────────
    Payload("123457", PayloadCategory.TR_DL, "Turkey DL (6 digits)"),
    Payload(
        "A1234568", PayloadCategory.TR_PASSPORT, "Turkey passport (letter + 7 digits)"
    ),
    ...
    Payload(
        ...
        PayloadCategory.FRL_DRAFT_NOT_CIRC,
        "Draft not for circulation",
    ),
    Payload(
        "Information Barrier", PayloadCategory.FRL_INFO_BARRIER, "Information barrier"
    ),
    Payload(
        "Inside Information", PayloadCategory.FRL_INSIDE_INFO, "Inside information"
    ),
    Payload(
        "Restricted List",
        PayloadCategory.FRL_INVEST_RESTRICTED,
        "Investment restricted list",
    ),
    Payload(
        "Market Sensitive", PayloadCategory.FRL_MARKET_SENSITIVE, "Market sensitive"
    ),
    Payload(
        "Pre-Decisional", PayloadCategory.FRL_PRE_DECISIONAL, "Pre-decisional document"
    ),
    # ── Latin America — Argentina ────────────────────────────────────────────
    Payload("20-12345678-9", PayloadCategory.AR_CUIL_CUIT, "Argentina CUIL/CUIT"),
    Payload(
        "AAA123457",
        PayloadCategory.AR_PASSPORT,
        "Argentina passport (3 letters + 6 digits)",
    ),
    # ── Latin America — Brazil ────────────────────────────────────────────────
    Payload(
        "12345678902", PayloadCategory.BR_CNH, "Brazil CNH driver's licence (11 digits)"
    ),
    Payload(
        "AB123457",
        PayloadCategory.BR_PASSPORT,
        "Brazil passport (2 letters + 6 digits)",
    ),
    Payload("12.345.679-9", PayloadCategory.BR_RG, "Brazil RG identity document"),
    Payload(
        "100123456789001", PayloadCategory.BR_SUS, "Brazil SUS health card (15 digits)"
    ),
    # ── Latin America — Chile ────────────────────────────────────────────────
    ...
    Payload(
        "123456790",
        PayloadCategory.PE_CARNET_EXT,
        "Peru carnet extranjeria (9-12 digits)",
    ),
    Payload(
        "AB123457",
        PayloadCategory.PE_PASSPORT,
        "Peru passport (2 letters + 6-7 digits)",
    ),
    Payload(
        "10123456790",
        PayloadCategory.PE_RUC,
        "Peru RUC (starts 10/15/17/20 + 9 digits)",
    ),
    ...
    # ── Loan and Mortgage ────────────────────────────────────────────────────
    Payload(
        "LOAN12345", PayloadCategory.LOAN_NUM_SHORT, "Loan number (8-15 alphanumeric)"
    ),
    ...
    Payload(
        "AB1234568",
        PayloadCategory.DEA_NUMBER,
        "US DEA registration (2 letters + 7 digits)",
    ),
    Payload(
        "ABC123456789",
        PayloadCategory.HEALTH_PLAN_ID,
        "Health plan ID (3 letters + 9 digits)",
    ),
    ...
    # ── Middle East — Iran ────────────────────────────────────────────────────
    Payload(
        "A12345679", PayloadCategory.IR_PASSPORT, "Iran passport (letter + 8 digits)"
    ),
    # ── Middle East — Iraq ────────────────────────────────────────────────────
    ...
    # ── Middle East — Jordan ──────────────────────────────────────────────────
    Payload(
        "A1234568", PayloadCategory.JO_PASSPORT, "Jordan passport (letter + 7 digits)"
    ),
    # ── Middle East — Kuwait ──────────────────────────────────────────────────
    ...
    # ── Middle East — Qatar ───────────────────────────────────────────────────
    Payload(
        "A1234568", PayloadCategory.QA_PASSPORT, "Qatar passport (letter + 7 digits)"
    ),
    # ── Middle East — Saudi Arabia ────────────────────────────────────────────
    Payload("A12345679", PayloadCategory.SA_PASSPORT, "Saudi Arabia passport"),
    # ── Middle East — UAE ─────────────────────────────────────────────────────
    Payload(
        "A12345679", PayloadCategory.UAE_PASSPORT, "UAE passport (letter + 7-9 digits)"
    ),
    ...
    Payload(
        ...
        "Canada NEXUS traveller number (9 digits)",
    ),
    Payload(
        "AB12345679",
        PayloadCategory.CA_PR_CARD,
        "Canada PR card (2 letters + 7-10 digits)",
    ),
    # ── North America — Mexico (new sub-patterns) ──────────────────────────────
    Payload(
        "GOMJUA78092H010", PayloadCategory.MX_CLAVE_ELECTOR, "Mexico clave de elector"
    ),
    ...
    Payload(
        "M12345679", PayloadCategory.MX_PASSPORT, "Mexico passport (letter + 8 digits)"
    ),
    ...
    Payload(
        "C12345679", PayloadCategory.US_PASSPORT_CARD, "US passport card (C + 8 digits)"
    ),
    # ── Personal / Postal ────────────────────────────────────────────────────
    ...
    Payload(
        "Litigation Hold",
        PayloadCategory.PRIV_LITIGATION_HOLD,
        "Litigation hold notice",
    ),
    Payload(
        "Privileged Information",
        PayloadCategory.PRIV_PRIVILEGED_INFO,
        "Privileged information marker",
    ),
    Payload(
        "Privileged and Confidential",
        PayloadCategory.PRIV_PRIV_CONF,
        "Privileged and confidential",
    ),
    Payload(
        "Protected by Privilege",
        PayloadCategory.PRIV_PROTECTED,
        "Protected by privilege",
    ),
    Payload(
        "Work Product Doctrine",
        PayloadCategory.PRIV_WORK_PRODUCT,
        "Work product doctrine",
    ),
    ...
    Payload(
        "12345678901236", PayloadCategory.REG_FINCEN, "FinCEN report number (14 digits)"
    ),
    Payload(
        "12346", PayloadCategory.REG_OFAC, "OFAC SDN number (4-6 digits, context req)"
    ),
    Payload(
        "12345678901234568", PayloadCategory.REG_SAR, "SAR filing number (14-20 digits)"
    ),
    ...
    Payload(
        "780087102", PayloadCategory.CUSIP_NUM, "CUSIP — Royal Bank of Canada (RY)"
    ),
    # CINS — CUSIP International Numbering System (non-US; first char is country letter)
    Payload(
        "G0177J108", PayloadCategory.CINS_NUM, "CINS — UK-listed security (G prefix)"
    ),
    Payload(
        "F22797108",
        PayloadCategory.CINS_NUM,
        "CINS — France-listed security (F prefix)",
    ),
    ...
    Payload("HWUPKR0MPOU8FGXBT394", PayloadCategory.LEI_NUM, "LEI — Apple Inc"),
    Payload(
        "8I5DZWZKVSZI1NUHU748", PayloadCategory.LEI_NUM, "LEI — JPMorgan Chase & Co"
    ),
    Payload("R0MUWSFPU8MPRO8K5P83", PayloadCategory.LEI_NUM, "LEI — BNP Paribas SA"),
    # Ticker symbols — exchange-qualified and bare
    Payload(
        "$AAPL", PayloadCategory.TICKER_SYMBOL, "Ticker — Apple (Nasdaq, $ prefix)"
    ),
    Payload(
        "AAPL",
        PayloadCategory.TICKER_SYMBOL,
        "Ticker — Apple bare symbol (context req)",
    ),
    Payload("JPM", PayloadCategory.TICKER_SYMBOL, "Ticker — JPMorgan Chase (NYSE)"),
    Payload(
        "RY.TO", PayloadCategory.TICKER_SYMBOL, "Ticker — Royal Bank of Canada (TSX)"
    ),
    Payload(
        "BRK.A", PayloadCategory.TICKER_SYMBOL, "Ticker — Berkshire Hathaway A (NYSE)"
    ),
    ...
    Payload(
        "3234936", PayloadCategory.VALOR_NUM, "Valor — Apple Inc on SIX Swiss Exchange"
    ),
    ...
    Payload(
        "CLORD20240517JPMC00000001",
        PayloadCategory.FIX_CL_ORD_ID,
        "FIX ClOrdID (tag 11) — JPMC order ref",
    ),
    Payload(
        "JPM-20240517-ORD001234",
        PayloadCategory.FIX_CL_ORD_ID,
        "FIX ClOrdID — alphanumeric format",
    ),
    # ── Social Media ─────────────────────────────────────────────────────────
    ...
    Payload(
        ...
        PayloadCategory.SUP_EXAM_FINDINGS,
        "MRA examination findings",
    ),
    Payload(
        "Non-Public Supervisory Information",
        PayloadCategory.SUP_NON_PUBLIC,
        "Non-public supervisory info",
    ),
    Payload(
        "Restricted Supervisory Information",
        PayloadCategory.SUP_RESTRICTED_SUP,
        "Restricted supervisory info",
    ),
    Payload(
        "Supervisory Confidential",
        PayloadCategory.SUP_SUPERVISORY_CONF,
        "Supervisory confidential",
    ),
    Payload(
        "Supervisory Controlled Information",
        PayloadCategory.SUP_SUPERVISORY_CTRL,
        "Supervisory controlled info",
    ),
    # ── URLs with Credentials ────────────────────────────────────────────────
    Payload(
        "https://api.example.com/data?token=abc123secret456def789",
        PayloadCategory.URL_WITH_TOKEN,
        "URL with embedded token parameter",
    ),
]
