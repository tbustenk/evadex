...
def _bar(pct: float, klass: str = "") -> str:
    class_attr = f"bar-fill {klass}".rstrip()
    return (
        f'<div class="bar-track"><div class="{class_attr}" '
...
<h2>Top Evasion Techniques (ranked by variants bypassed)</h2>
<p class="meta" style="margin-bottom:8px">Sorted by absolute count of variants the scanner missed. The Recommendations section below ranks separately by <em>evasion rate</em>, with a 10-sample minimum, so the two lists may differ.</p>
...
<footer>
Generated by evadex v{_evadex_version()} — DLP evasion test suite.
