...
@click.option("--port", default=8081, show_default=True, type=int, help="TCP port.")
@click.option(
    "--api-key",
    default=None,
    metavar="KEY",
    help=(
        "Require this key in the x-api-key header on every "
...
@click.option(
    ...
    metavar="ORIGINS",
    help=(
        "Comma-separated CORS allow-list. Default '*' allows "
...
@click.option(
    ...
    type=click.Path(),
    help=(
        "Directory evadex scans against (defaults to the "
...
@click.option(
    ...
    type=click.Path(),
    help=(
        "Path to the siphon binary. Highest-priority entry "
...
@click.option(
    ...
    metavar="STYLE",
    help=(
        "Default scanner command style forwarded as "
...
def bridge(
    ...
    click.echo(
        ...
        + (
            "x-api-key required"
            if os.environ.get("EVADEX_BRIDGE_KEY")
