...
- **`evadex falsepos` now auto-discovers `evadex.yaml`** the same way `evadex scan` does — `--config` flag added, and CLI flags continue to override config values. Previously a profile run (`evadex profile run banking-daily`) would succeed on the scan step (which auto-discovers) but fail on the falsepos step with `Health check failed for adapter 'siphon-cli'. Is siphon installed and on PATH?` whenever the scanner binary lived outside `PATH`. Built-in profiles don't specify `exe:` (it's machine-local), so a single `evadex.yaml` now serves both steps of a profile run.

### Changed

- **HTML report (`evadex report`)** — three CISO-grade polish improvements in `src/evadex/cli/commands/report.py`:
  - Footer now shows the producing version: *"Generated by evadex v3.25.5 — …"* (pulled from `importlib.metadata`). Readers can pin which methodology produced the report.
  - The *Top Evasion Techniques* section is now titled *"Top Evasion Techniques (ranked by variants bypassed)"* with an explanatory note clarifying that the Recommendations section ranks by evasion **rate** with a 10-sample minimum — so the two lists may legitimately differ.
  - `_bar()` no longer emits `class="bar-fill "` (trailing space, empty class) when no severity qualifier applies; outputs clean `class="bar-fill"`.

### Verified

- **Profile runs end-to-end:**
  - `banking-daily` (built-in) — scan completes; falsepos now completes (previously failed health-check before the fix).
  - `northam-daily` (new user profile) — scan + falsepos both complete; `last_run` is written to the profile YAML; schedule export emits valid `cron` and Windows Task Scheduler XML.
- **`evadex scan --save-as test-profile`** writes a valid user profile and `evadex profile show test-profile` round-trips it.
- 1299/1299 tests pass (unit + integration).
...
- **`northam` tier — new default** replacing `banking`:
  - Covers North America: all Canadian identifiers from `banking` plus US national identifiers (SSN, US_DL, US_ITIN, US_EIN, US_MBI, US_PASSPORT, US_PASSPORT_CARD, US_ZIP4, US_DEA, US_NPI, US_KTN) and a full capital markets identifier set (CUSIP, SEDOL, FIGI, LEI, REUTERS_RIC, TICKER_SYMBOL, MIFID_TX_ID, VALOR_NUM, WKN_NUM) plus regulatory categories (MERS_MIN, REG_CTR, REG_FINCEN, REG_SAR).
  - 102 categories total (71 from `banking` + 31 new).
  - `NORTHAM_TIER` frozenset exported from `evadex.payloads.tiers`; `get_tier_categories("northam")` dispatches to it.
  - `VALID_TIERS` updated in both `tiers.py` and `config.py`.
  - `evadex scan`, `evadex generate`, `evadex quickstart`, and `evadex init` all default to `northam`.
  - `evadex doctor` now reports `✓ Default tier: northam (North America — Canada + US, 102 categories)`.
  - `evadex.yaml` generated by `evadex quickstart` uses `tier: northam`.
  - README tier table updated; `northam` listed first as default.

### Tests

- 57 new tests in `tests/unit/payloads/test_northam_tier.py`: tier registry, superset relationship, all provincial health cards, all provincial/territorial DLs, CA banking identifiers, US national IDs, capital markets identifiers, payment identifiers, PII, CLI acceptance.
