...
### `evadex ci` runs scan as a subprocess (v3.31.0)
`evadex ci` calls `subprocess.run([sys.executable, "-m", "evadex", "scan", ...])` and parses JSON from stdout. Exit codes: 0 = all thresholds met, 1 = threshold violated, 2 = scan process error or bad JSON. The underlying scan runs `--no-stream` (batch mode) so stdout contains a complete JSON blob. Only the first 6 tokens of the command are shown in CI output (api_key not echoed). Tests mock `evadex.cli.commands.ci.subprocess.run`.

### `http_generic` adapter config is flat, not nested (v3.31.0)
`BaseAdapter.from_dict(d)` puts all keys not in `(base_url, api_key, timeout)` into `config.extra`. So `url`, `method`, `request_field`, `response_path`, and `auth_header` must be top-level keys in the config dict — **not** nested under an `extra` sub-key. In tests, construct via `HttpGenericAdapter({"url": "...", "request_field": "content"})`. In `evadex.yaml`, the fields are already top-level.

### Unit tests that invoke the CLI must not pick up the local evadex.yaml (v3.29.0, fixed in refinement pass)
The local `evadex.yaml` in the repo root sets `tool: siphon-cli`, `transport: http`, `url: http://localhost:8091`. Unit tests that use `CliRunner` without `isolated_filesystem()` or `monkeypatch.chdir()` pick up this file and fail when the siphon-api is not running. **Fixed:** `test_streaming_engine.py` and `test_northam_tier.py` now wrap `runner.invoke` in `with runner.isolated_filesystem()`. New CLI command tests (`test_ci_command.py`, `test_benchmark_command.py`) also use `isolated_filesystem()`. CI always passes because it has no local `evadex.yaml`.
